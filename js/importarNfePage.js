      ...
      function setupEventListeners() {
        ...
        document
          .getElementById('btnAddItemFromEmpenho')
          ?.addEventListener('click', abrirModalItensEmpenho);
        ...
        try {
          ...
          if (manager && typeof manager.buscarEmpenhos === 'function') {
            try {
              ...
              console.log(
                `[NF] carregarEmpenhos via dbManager: ${todosEmpenhos.length} total, ${empenhos.length} validados`
              );
          ...
          req.onsuccess = () => {
            ...
            console.log(
              `[NF] carregarEmpenhos local: ${todosEmpenhos.length} total, ${empenhos.length} validados`
            );
      ...
      function selecionarEmpenho(id) {
        ...
        document
          .querySelectorAll('.empenho-card.selected')
      ...
      function setAllChecks(checked) {
        document
          .querySelectorAll('#modalItensEmpenhoLista .chkItemEmpenho:not(:disabled)')
          .forEach((chk) => {
            chk.checked = checked;
          });
      }
      ...
      function itemKey(it) {
        ...
        if (ic) {return `IC:${ic.padStart(3, '0')}`;}
      ...
      function confirmarItensEmpenho() {
        ...
        const checks = Array.from(
          document.querySelectorAll('#modalItensEmpenhoLista .chkItemEmpenho')
        );
      ...
      function coletarItens() {
        ...
        rows.forEach((row, idx) => {
          ...
          const unidade = (row.querySelector(`[name="unidade_${id}"]`)?.value || '')
            .trim()
            .toUpperCase();
      ...
      function validarFormulario() {
        ...
        for (const item of itens) {
          if (/^\d+$/.test(item.unidade)) {
            mostrarAlerta(
              `Item "${item.descricao}": unidade "${item.unidade}" não pode ser apenas numérica`,
              'danger'
            );
            valido = false;
            break;
      ...
      function atualizarEstadoBotaoSalvar() {
        ...
        btnSalvar.title =
          ultimaValidacao.warnings.length > 0 ? 'Há alertas, mas pode salvar' : 'Validação OK';
        btnSalvar.innerHTML =
          ultimaValidacao.warnings.length > 0 ? '⚠️ Salvar (com alertas)' : '✅ Salvar Nota Fiscal';
      }
      ...
      function formatarCNPJ(valor) {
        ...
        if (nums.length <= 5) {return nums.replace(/^(\d{2})(\d)/, '$1.$2');}
        if (nums.length <= 8) {return nums.replace(/^(\d{2})(\d{3})(\d)/, '$1.$2.$3');}
        if (nums.length <= 12) {return nums.replace(/^(\d{2})(\d{3})(\d{3})(\d)/, '$1.$2.$3/$4');}

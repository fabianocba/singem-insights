...
export default function SolicitacaoTransportes() {
  ...
  const solicitacoes = getPorTipo('transportes');
  ...
  const handleSalvar = (enviar: boolean) => {
    ...
    const nova = adicionar({
      ...
      veiculoDesignado: '',
      motoristaDesignado: '',
      itens: [{ id: '1', descricao: `Viagem a ${form.destino}`, unidade: 'viagem', quantidade: 1 }],
    });
    setForm({ solicitante: '', setor: '', destino: '', motivo: '', dataViagem: '', horaPartida: '', horaRetorno: '', passageiros: '', tipoVeiculo: '', observacao: '' });
    setModalNova(false);
    if (enviar) {
      toast.success(`${nova.numero} enviada para Transportes`);
      adicionarNotificacao({ tipo: 'sol_veiculo', titulo: `${nova.numero} Enviada`, mensagem: `Solicitação de veículo enviada à Coordenação de Transportes.`, link: '/solicitacoes/transportes' });
    } else {
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-3 flex-1">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{s.motivo || s.justificativa}</p>
          ...
          {detalheSol && (
            <>
              ...
              <div className="space-y-4">
                <div>
                  <p className="font-semibold">{detalheSol.destino}</p>
                  <p className="text-sm text-muted-foreground">{detalheSol.motivo}</p>
                </div>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div><span className="text-muted-foreground">Data viagem:</span> <span className="font-medium">{detalheSol.dataViagem}</span></div>
                  <div><span className="text-muted-foreground">Horário:</span> <span className="font-medium">{detalheSol.horaPartida} - {detalheSol.horaRetorno}</span></div>
                  <div><span className="text-muted-foreground">Passageiros:</span> <span className="font-medium">{detalheSol.passageiros}</span></div>
                  <div><span className="text-muted-foreground">Tipo veículo:</span> <span className="font-medium">{detalheSol.tipoVeiculo}</span></div>
                  {detalheSol.veiculoDesignado && <div><span className="text-muted-foreground">Veículo:</span> <span className="font-medium text-emerald-400">{detalheSol.veiculoDesignado}</span></div>}
                  {detalheSol.motoristaDesignado && <div><span className="text-muted-foreground">Motorista:</span> <span className="font-medium text-emerald-400">{detalheSol.motoristaDesignado}</span></div>}
                ...
                <div className="rounded-lg bg-muted/30 border border-border/50 p-3 text-xs text-muted-foreground">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span>→</span>
                    <span className={['enviada','aprovada','atendida'].includes(detalheSol.status) ? 'text-emerald-400' : ''}>Enviada a Transportes</span>
                    <span>→</span>
                    <span className={['aprovada','atendida'].includes(detalheSol.status) ? 'text-emerald-400' : detalheSol.status === 'rejeitada' ? 'text-red-400' : ''}>
                      {detalheSol.status === 'rejeitada' ? 'Rejeitada' : 'Aprovada + Veículo'}
                    </span>
                    <span>→</span>
                    <span className={detalheSol.status === 'atendida' ? 'text-emerald-400' : ''}>Viagem Realizada</span>
                  </div>
                </div>

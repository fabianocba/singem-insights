          ...
          {editando ? (
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
                ...
                <div><Label>Processo SUAP</Label><div className="flex gap-2"><Input value={form.objeto || ''} onChange={e => setForm(p => ({ ...p, objeto: e.target.value }))} /><a href={form.objeto ? `https://suap.ifbaiano.edu.br/processo_eletronico/processo/${form.objeto}/` : '#'} target="_blank" rel="noopener noreferrer" className={`inline-flex items-center justify-center h-10 px-3 rounded-md text-sm font-medium border border-input bg-background hover:bg-accent hover:text-accent-foreground transition-colors whitespace-nowrap ${!form.objeto ? 'pointer-events-none opacity-50' : ''}`}>Consultar</a></div></div>
                <div>

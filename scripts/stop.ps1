...
$deploySteps = @(
  ...
  "HTML_STATUS=`$(curl -sS -o /tmp/singem_home.out -w '%{http_code}' $SiteUrl/ || true)"
  "echo [INFO] HTML status: `$HTML_STATUS"
  "if [ `"`$HTML_STATUS`" != '200' ]; then echo '[ERRO] Smoke HTML retornou status != 200'; exit 8; fi"
  "grep -q 'SINGEM v' /tmp/singem_home.out || { echo '[ERRO] Smoke HTML sem marcador de versionamento (SINGEM v)'; exit 9; }"
  "echo [OK] HTML contém marcador de versionamento"
  "echo [INFO] Smoke: /api/version"
  "API_STATUS=`$(curl -sS -o /tmp/singem_api_version.out -w '%{http_code}' $SiteUrl/api/version || true)"
  "echo [INFO] /api/version status: `$API_STATUS"
  "head -c 400 /tmp/singem_api_version.out || true"
  "echo"
  "echo [INFO] Smoke: /health"
  "HEALTH_STATUS=`$(curl -sS -o /tmp/singem_health.out -w '%{http_code}' $SiteUrl/health || true)"
  "echo [INFO] /health status: `$HEALTH_STATUS"
  "head -c 400 /tmp/singem_health.out || true"
  "echo"
)
